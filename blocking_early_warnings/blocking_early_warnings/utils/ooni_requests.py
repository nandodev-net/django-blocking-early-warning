...
from blocking_early_warnings.settings   import OONI_ENDPOINT, DATE_FORMAT, COUNTRY_CODE, NUMBER_OF_HOURS
...
def sync_db_metrics():
    ...
    # Process data
    metrics = map(lambda k : (k[0], compute_metrics(k[1], since=get_hour(yesterday))), data.items())
    ...
    def update_metrics(metrics):
        Metric.objects.bulk_update(metrics, ["hour", "anomaly_count", "measurement_count"])
    ...
    for m in metrics:
        ...
        if (amnt := curr_metrics.count()) != NUMBER_OF_HOURS:
            raise InvalidQuery(f"Inconsistent database: There should be {NUMBER_OF_HOURS} metrics instance for url {url} and asn {asn}, but {amnt} were found")
...
def compute_metrics(measurements : List[Dict[str, Any]], since : datetime) -> Dict[datetime, Dict[str, int]]:
    """
...
def get_raw_data_from_ooni( since : datetime, 
                            until : datetime, 
